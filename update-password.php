        ...
        if($op==$pw)
        {
         if($op!=$np)
         {
             if($nps==$np)
             {
                ...
                $nps=base64_encode($nps);#加密
                $abnps=base64_decode($nps);
                $u_acc=$_COOKIE['Email'];
                $UpdateSql="UPDATE users SET password='$nps' WHERE Email='$u_acc'";
                $result=$connect->query($UpdateSql);
                $dd="密碼修改成功，請重新登入";
                header('refresh:1;url=http://localhost/Workfile/dash-edit-password.php');
             }
             else
             {
                $dd="新密碼與再次新密碼不同請重新輸入";
