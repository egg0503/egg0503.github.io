...
<?php
		...
		if(($_POST["First_Name"]!=null)&&($_POST["Last_Name"]!=null)&&($_POST["Email"]!=null)&&($_POST["password"]!=null))
		{
			if(isset($_POST['decide'])) 
			{
					...
					$gender=$_POST["Gender"];
					$password=base64_encode($password);#加密
					$abpassword=base64_decode($password);#解密
					$insertSql = "INSERT INTO users (First_name,Last_name,Phone,Email,Creatday,password,Gender,Birthday) VALUES ('$fname','$lname','$phone','$email',NOW(),'$password','$gender','$date')";	
					$status = $connect->query($insertSql);
					$timeOutLimit = "20";  //設定時間間斷		
					if ($status) 
					{
						...
						{  //判斷是否有session
							...
							{   
								$dd="請在20秒內不要重新整理及送出表單，，將於3秒後跳轉";
							//如果有session 且session時間間隔小於5分鐘 直接die()退出並輸出提示資訊。
							   die("請在20秒內不要重新整理及送出表單，，將於3秒後跳轉註冊頁面");
							   header("Refresh:3;url=http://localhost/Workfile/signup.html");
							} else 
							{    //如果沒有超時，將session時間重置為當前時間。
							   $_SESSION["timeout"] = time();
							   $dd="註冊成功5秒後跳轉登入頁面";
							  
							   header("Refresh:5;url=http://localhost/Workfile/signin.html");
							}
						} 
						else 
						{
							$_SESSION["timeout"] = time();
							$dd="註冊成功5秒後跳轉登入頁面";							
							//如果沒有timeout的session，則建立session為當前時間，便於下次存取頁面時判斷
                            header("Refresh:5;url=http://localhost/Workfile/signin.html");
						}
					}
					else
					{
						$dd="註冊失敗，電子郵件重複，請更換一組未使用過的，將於三秒後跳轉";
						header("Refresh:3;url=http://localhost/Workfile/signup.html");
					}
			}
		}
		...
		{
			$dd="請填入完整資料，5秒後跳回註冊網頁";
			echo "請填入完整資料，5秒後跳回註冊網頁";	
			header("Refresh:5;url=http://localhost/Workfile/signup.html");
                                    ...
                                    <li class="has-dropdown">
                                        ...
                                        <ul style="width:170px">
                                            <li class="left-100">
                                                ...
                                                <a href="index.php">主頁<i class="fas i-state-right u-s-m-l-6"></i></a>
                                            ...
                                            <!--li class="has-dropdown has-dropdown--ul-left-100">
                                                ...
                                                <ul style="width:200px">
                                                    ...
                                                    <li>
                                                        ...
                                                        <a href="shop-side-version-2.php">Shop Side Version 2</a></li>
                                                </ul>
                                                <!--====== End - Dropdown ======-->
                                            <!--li class="has-dropdown has-dropdown--ul-left-100">
                    ...
                    <div class="container">
                        <div class="row">
                            <div class="col-lg-12">
                                ...
                                <h1 class="section__heading u-c-secondary"><big><?php echo "$dd";?></big></h1>
                                </div>
                            </div>
                        </div>
                    </div>
